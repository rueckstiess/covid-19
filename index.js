...
const CONFIRMED_FILE = "2019-nCoV/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Confirmed.csv";
...
Promise.all(promises)
    .then(tables => {
        ...
        MongoClient.connect(MONGO_URI, { useUnifiedTopology: true }, function(err, client) {
            ...
            const collection = client.db(DATABASE_NAME).collection('statistics');
            collection
                .drop() // !!! This drops the collection and recreates it !!!
                .then(() => collection.insertMany(docsToInsert))
                .then(res => {
                    console.log(`Successfully inserted ${res.result.n} documents into collection.`);
                    client.close();
                });
        });
    })
